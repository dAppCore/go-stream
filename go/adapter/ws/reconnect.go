...
func (client *ReconnectingClient) Connect(ctx context.Context) core.Result {
	if client == nil {
		return core.Fail(core.E("stream.ws", "nil reconnecting client", nil))
	}
	if ctx == nil {
...
func (client *ReconnectingClient) Send(msg stream.Message) core.Result {
	if client == nil {
		return core.Fail(core.E("stream.ws", "nil reconnecting client", nil))
	}
	...
	if conn == nil {
		return core.Fail(core.E("stream.ws", "not connected", nil))
	}
	client.mutex.Lock()
	...
	return core.ResultOf(nil, client.conn.WriteMessage(websocket.TextMessage, payload.Value.([]byte)))
}
...
// _ = client.Close()
func (client *ReconnectingClient) Close() core.Result {
	if client == nil {
		return core.Ok(nil)
	}
	client.mutex.Lock()
...
func (client *ReconnectingClient) readLoop(ctx context.Context, conn *websocket.Conn) core.Result {
	for {
		select {
		case <-ctx.Done():
