...
func (adapter *Adapter) serveHTTP(w http.ResponseWriter, r *http.Request, channels []string) {
	...
	for {
		...
		case stream.TypeSubscribe:
			if r := adapter.hub.SubscribePeer(peer, message.Channel); !r.OK {
				if ok := peer.Send(marshalMessage(stream.Message{
					Type:      stream.TypeError,
					Channel:   message.Channel,
					Data:      errorPayload(r.Value),
		...
		default:
			if message.Channel == "" {
				if r := adapter.hub.BroadcastFromPeer(peer, payload); !r.OK {
					return
				}
				continue
			}
			if r := adapter.hub.PublishFromPeer(peer, message.Channel, payload); !r.OK {
				return
			}
		}
	}
...
func errorPayload(value any) map[string]any {
	if value == nil {
		return nil
	}
	...
	return map[string]any{"message": core.Sprintf("%v", value)}
}
